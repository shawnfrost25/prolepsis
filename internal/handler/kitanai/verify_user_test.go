...
func TestVerifyRegistration(t *testing.T) {
	...
	err = lib.RunMake("../../../", "redis-test", "REDIS_PASS="+redisPSWD, "REDIS_PATH=internal/handler/testdata/create_pending_registration.redis")
	if err != nil {
		t.Fatalf("Couldn't successfully create mock session token, due to error: %v", err)
	}
	defer func() {
		err = lib.RunMake("../../../", "redis-test", "REDIS_PASS="+redisPSWD, "REDIS_PATH=internal/handler/testdata/clean_pending_registration.redis")
		if err != nil {
